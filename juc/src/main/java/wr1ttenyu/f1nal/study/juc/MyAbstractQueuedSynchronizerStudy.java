...
/**
 * 提供一个依赖FIFO等待队列实现的阻塞式锁和相关的同步器（信号量、事件等）的框架
...
public abstract class MyAbstractQueuedSynchronizerStudy
     ...
     */
    protected MyAbstractQueuedSynchronizerStudy() { }
    ...
    private static boolean shouldParkAfterFailedAcquire(Node pred, Node node) {
        ...
        if (ws > 0) {
            /*
             * 前去节点被取消了 跳过前驱节点 直到找到一个不是取消状态的前驱节点
            ...
            /*
             * 这是前驱节点肯定是初始0或者共享状态-3
             * 表明我们需要一个 signal 但是不要现在阻塞
             * waitStatus must be 0 or PROPAGATE.  Indicate that we
             * 调用者需要尝试去确认在阻塞前确实不能获取锁
